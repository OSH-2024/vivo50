...
def download_file():
    if request.method == 'POST':
        ...
        print(path)
        try:
            return send_file(storage_path + path, as_attachment=True)
        except Exception as e:
            message_forward('下载失败！')
            print('从central server下载失败:', e)
            return redirect(url_for('index'))
    return redirect(url_for('index'))
...
def new_dir():
    if request.method == 'POST':
        ...
        folder_path = os.path.join(storage_path + path, dir_name)
        print(folder_path)
        os.makedirs(tmp_path)
        os.makedirs(folder_path)    #直接在juicefs的目录下创建文件夹
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=visit_web_port, threaded=True)

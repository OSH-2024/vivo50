...
def upload_file():
    if request.method == 'POST':
        ...
        if central_server.upload_to_central(fileid, filename, file, path):
...
def delete_file():
    ...
    #print(filename)
    if is_dir == "false":
        central_server.Delete_to_central(fileid, filename, path)
...
def search():
    if request.method == 'POST':
        ...
        print('查询的问题是：')
        print(query)
        if central_server.Search_to_central(query) :

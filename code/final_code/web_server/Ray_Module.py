...
def Upload(fileid,filename,filepath):
    ...
    send_data="Upload"+split_char+str(fileid)+split_char+str(filename)+split_char+str(filepath) + split_char + str(keywords)
    print("关键字是")
    print(keywords)
    # print("------")
    try:
...
def Delete(fileid,filename,filepath):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    send_data = "Delete" + split_char + str(fileid) + split_char + str(filename) + split_char + str(filepath)
    print("发送的删除信息是：")
    print(send_data)
    try:
        # 连接目标主机

...
absolute_path=settings["absolute_path"]   #存储下载文件的路径
...
DOWNLOAD_FOLDER = absolute_path  # DOWNLOAD_FOLDER 将用于存储下载的文件
...
def upload_file():
    if request.method == 'POST':
        path = request.form.get('path', '')  # 这个path指的是json中的路径
        ...
        if connect_to_central.upload_to_central(id, filename, file, path):
...
def download_file():
    if request.method == 'POST':
       ...
       # print(path)
        ...
        id = int(element_id[7:])
        print('id:',id)
        if connect_to_central.download_to_central(id, filename, target_path, path):
